...
>
> Este documento asume que ya leíste `BLUEPRINT.md` (arquitectura, puertos, nombres exactos de recursos) y que el repo tiene los 5 microservicios ya construidos (`backend/{users-address,products,ventas-resenas,ordenes,analitica}`) y el frontend en `frontend`.
...
4. **Monorepo**: si te lo pregunta, marca que la app está en una subcarpeta → **App root**: `frontend`.
5. Verifica que el **Build settings** detectado coincida con `amplify.yml` del repositorio (appRoot `frontend`, build command `npm run build` y artefactos `dist`). Si no lo detecta, pégalo manualmente en el editor.
...
(Esto es lo que ya trae `frontend/amplify-rewrites.json` — si Amplify te dejó importarlo directamente, mejor.)
